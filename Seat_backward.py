import Student_backward
import re
class Seat:
    def __init__(self):
        self.stu_no_list, self.class_dic_for_each, self.seat_list, self.seat_dic_for_each = Student_backward.Student().pre_process()
        self.search_seat_dic = {}


    def SearchDatenum(self,nowdate):
        weekdic = {'1': "Mon", '2': "Tues", '3': "Wed", '4': "Thur", '5': "Fri",'6':"Sat",'7':"Sun"}
    ...
    def search(self,time,classnum,seatnum_list):
        ...
        weeknum = week_list[1] #weeknum代表周几
        result_dict = {}
        vip_list = {}
        # for i in seatnum_list:
        #     if weeknum == 'Sat' or 'Sun':
        ...
        for i in seatnum_list:
            if i in self.seat_dic_for_each:
                ...
                vip_list.update({i:0})
        if weeknum in ['Mon','Tues','Wed','Thur','Fri']:
            for i in seatnum_list:
                if vip_list[i]!=0:
                    s = vip_list[i]
                    s1 = s[0]
                    vip_df = self.class_dic_for_each[s1].fillna(0)
                    df_class_time = vip_df[weeknum][classnum]
                    if df_class_time!=0:
                        df_class_time = vip_df[weeknum][1].split('\n')
                        week = Seat()
                        week_list = week.SearchWeeknum(df_class_time)
            ...
            for i in seatnum_list:
                if result_dict[i] == '空闲':
                    sss = vip_list[i]
                    ss1 = sss[0]
                    vip_df1 = self.class_dic_for_each[ss1].fillna(0)
                    for j in range(classnum, 6):
                        df_class_time_next = vip_df1[weeknum][j]
            ...
            for j in seatnum_list:
                if vip_list[j] != 0:
                    result_dict.update({j: '占用'})
                else:
                    result_dict.update({j: '未分配'})
        print(result_dict)
        return result_dict
